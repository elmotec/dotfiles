...
Set-Alias cm chezmoi -Option AllScope
...
# Autocomplete uv
$uvCompletionFile = Join-Path -Path $PSScriptRoot -ChildPath "uv-completion.ps1"
if (Get-Command uv -ErrorAction SilentlyContinue) {
    ...
    if (Test-Path $uvCompletionFile) {
        ...
        uv generate-shell-completion powershell | Out-File -FilePath $uvCompletionFile -Encoding utf8
        . $uvCompletionFile
    }
}
